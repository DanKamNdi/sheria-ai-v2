...
const ChatsPage = () => {
  ...
  const startChat = async (usrObj) => {
    ...
    try {
      ...
      router.push(`/trial/chat/${uuid}`, {scroll: false});
      setLoading(false)
    } catch (e) {
          ...
          <>
            ...
            <div className="w-full md:w-1/2">
              {chats.length > 0 ? (
                  ...
                  <ul className="divide-y divide-gray-200">
                    {chats.map((chat) => (
                      <li key={chat.id} className="py-4">
                        <Link href={`/trial/chat/${chat.chatId}`}>
                ...
                <div className="flex justify-center items-center">
                  <p className="md:text-xl font-bold">Chat with Sheria to see your conversations here.</p>
                </div>
              )}
            </div>
          </>
